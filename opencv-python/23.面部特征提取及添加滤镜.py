...
def createBox(img,points,scale=5,masked= False,cropped= True):
    if masked:
        ...
        mask = cv2.fillPoly(mask,[points],(255,255,255))  #填充多边形
    ...
    for face in faces:
        ...
        if len(myPoints) != 0:
            try:
                ...
                #imgOriginalGray = cv2.cvtColor(imgOriginalGray, cv2.COLOR_GRAY2BGR)   #改成相同通道
                imgColorLips = cv2.addWeighted(imgOriginal ,1,imgColorLips,0.4,0)   #必须是相同通道
